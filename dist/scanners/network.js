...
export class NetworkScanner {
    ...
    assessSeverity(port, risk) {
        if (risk.includes('ransomware') || risk.includes('EternalBlue') || risk.includes('BlueKeep') || risk.includes('Remote code execution'))
            return 'critical';
        if (risk.includes('No authentication') || risk.includes('Plaintext') || risk.includes('Brute force'))
            return 'high';
        if (risk.includes('misconfiguration') || risk.includes('exposure'))
            return 'medium';
        return 'low';
    }
    summarize(vulns) {
