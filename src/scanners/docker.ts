...
export class DockerScanner implements Scanner {
  ...
  private async scanContainer(id: string, vulns: Vulnerability[]) {
  ...
  private async scanImage(imageName: string, vulns: Vulnerability[]) {
    ...
    try {
      ...
      if (tags.some((t: string) => t.endsWith(':latest'))) {
        vulns.push({
