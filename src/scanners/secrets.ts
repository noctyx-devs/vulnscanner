...
export class SecretsScanner implements Scanner {
  ...
  private walkDir(dir: string, vulnerabilities: Vulnerability[]) {
    let entries: string[];
    try {
    ...
    for (const entry of entries) {
      ...
      if (stats.isDirectory()) {
        ...
        if (IGNORE_EXTS.has(extname(entry))) continue;
  ...
  private scanFile(filePath: string, vulnerabilities: Vulnerability[]) {
    let content: string;
    try {
