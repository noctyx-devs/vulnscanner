...
import type { Scanner, ScanTarget, ScanResult } from './types/index.js';
...
const ALL_SCANNERS: Record<string, new () => Scanner> = {
...
program
  ...
  .action(async (options) => {
    ...
    const selectedScanners = options.scanners
      ? options.scanners.split(',').map((s: string) => s.trim().toLowerCase())
